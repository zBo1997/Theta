...
import static com.momo.theta.constants.AuthConstants.BASIC_HEADER_PREFIX;
import static com.momo.theta.enums.ExceptionCodeEnum.JWT_BASIC_INVALID;
import static com.momo.theta.enums.ExceptionCodeEnum.JWT_PARSER_TOKEN_FAIL;

import cn.hutool.core.date.DateUtil;
import cn.hutool.core.text.StrPool;
import cn.hutool.core.util.StrUtil;
import com.momo.theta.constants.AuthConstants;
import com.momo.theta.enums.ExceptionCodeEnum;
import com.momo.theta.exception.BizException;
import com.momo.theta.exception.UnauthorizedException;
...
public final class JwtUtil {
     ...
     */
    private static final String BASE64_SECURITY = Base64.getEncoder().encodeToString(
        AuthConstants.JWT_SIGN_KEY.getBytes(StandardCharsets.UTF_8));
    ...
    public static Token createJwt(Map<String, String> user, long expire) {
        ...
        tokenInfo.setExpiration(DateUtil.toLocalDateTime(exp));
        return tokenInfo;
    }
    ...
    public static Claims parseJwt(String jsonWebToken, long allowedClockSkewSeconds) {
        try {
            ...
            //过期
            throw new UnauthorizedException(ExceptionCodeEnum.JWT_TOKEN_EXPIRED.getCode(), ExceptionCodeEnum.JWT_TOKEN_EXPIRED.getMsg(),ex);
        } catch (SignatureException ex) {
            log.error("token=[{}] 签名错误", jsonWebToken, ex);
            //签名错误
            throw new UnauthorizedException(ExceptionCodeEnum.JWT_SIGNATURE.getCode(), ExceptionCodeEnum.JWT_SIGNATURE.getMsg(), ex);
        } catch (IllegalArgumentException ex) {
            log.error("token=[{}] 为空", jsonWebToken, ex);
            //token 为空
            throw new UnauthorizedException(ExceptionCodeEnum.JWT_ILLEGAL_ARGUMENT.getCode(), ExceptionCodeEnum.JWT_ILLEGAL_ARGUMENT.getMsg(), ex);
        } catch (Exception e) {
    ...
    public static String getToken(String token) {
        ...
        if (token.startsWith(AuthConstants.BEARER_HEADER_PREFIX)) {
            return StrUtil.subAfter(token, AuthConstants.BEARER_HEADER_PREFIX, false);
        }
    ...
    public static Claims getClaims(String token, long allowedClockSkewSeconds) {
        ...
        if (token.startsWith(AuthConstants.BEARER_HEADER_PREFIX)) {
            String headStr = StrUtil.subAfter(token, AuthConstants.BEARER_HEADER_PREFIX, false);
